...
// IN THIS PROBLEM YOU NEED TO TAKE BOTH INTO CONSIDERETION WIDTH AND HEIGHT
// IN FIRT APPROCH I WAS ONLY CONSIDERING HEIGHT WHICH IS WRONG
// THE BEST APPROCH IS QUITE HARD TO COMES INTO MIND WHICH IS TWO POINTER BUT FIRST
// DO THE BRUTE FORCE SOLUTION WHICH IS NESTED LOOP WHERE YOU COMPARE EACH POINT WITH ALL OTHER POINTS
// BRUTE FORCE WILL FAIL IN LEET CODE WITH TIMELIMIT EXCEED ERROR.
function containerWithMostWaterLeetcodeBF(height){
  let water = 0;    
  for(let i = 0; i < height.length -1 ; i++){
    for(let j = i + 1 ; j < height.length ; j++){
      let minHeight = Math.min(height[i],height[j])
      let width = j - i 
      if(minHeight * width > water){
...
// i at starting and j at the end 
// get water by calculating minHeight * width => water
// Now this is the tricky part whom should we move so always move where we have min height 
// So suppose at i we have height 10 and at j we have height 30 then move i because see if
// you move i the there are chances that you get bigger height the min but in both case width
// will be decress by one : I KNOW HARD TO UNDERSTAND BUT DO IT COUPLE OF TIME
// https://www.youtube.com/watch?v=qUDp8IUbZto
function containerWithMostWaterLeetcode11(height){
 let i = 0;
